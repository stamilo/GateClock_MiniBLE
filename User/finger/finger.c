...
u16 Add_Finger(u16* user_id) {
    ...
    Usart_SendFinger_ADD_Success(USART3, 0, 0);

    /******************************* 采集3次指纹信息 *******************************/
    for (u8 i=0; i<3; i++) {
        // 采集指纹图像，如果超时就向手机发送数据包头【ERROR_CODE_TIMEOUT】，并且return退出
        if (QS808_Wait(10000) == QS808_WAIT_TIMEOUT) {
            Usart_SendFinger_ADD_Error(USART3, i+1, ERROR_CODE_TIMEOUT);
        ...
        // 如果指纹质量差，就向手机发送数据包头【ERROR_CODE_FINGERBAD】，并且return退出
            ...
            Usart_SendFinger_ADD_Error(USART3, i+1, ERROR_CODE_FINGERBAD);
        ...
        // 如果能跑到这里就说明采集成功，就向手机端发送数据包头【ERROR_CODE_SUCCESS】
            ...
            if(i<2)    Usart_SendFinger_ADD_Success(USART3, i+1, 0);
        }
	...
	if ((temp == QS808_DATA_ERR) || (temp == QS808_UNKNOW_ERR) || (temp == ERR_INVALID_BUFFER_ID) || (temp == ERR_GEN_COUNT) || (temp == ERR_MERGE_FAIL)) {
        Usart_SendFinger_ADD_Error(USART3, 3, ERROR_CODE_FINGERBAD);
		return ERROR_CODE_FINGERBAD;
    }
	...
	if ((temp == QS808_DATA_ERR) || (temp == QS808_UNKNOW_ERR) || (temp == ERR_INVALID_PARAM)) {
        Usart_SendFinger_ADD_Error(USART3, 3, ERROR_CODE_TIMEOUT);
	    return ERROR_CODE_TIMEOUT;
    }
	...
	if (temp == ERR_DUPLICATION_ID)	{
        Usart_SendFinger_ADD_Error(USART3, 3, ERROR_CODE_DUPLICATION);
    ...
    for (u16 i=0; i<MY_USER_MAX_NUM; i++) {
...
u16 Delete_Finger(u16 user_number) {
	...
	if (user_number<MY_USER_MAX_NUM && user_number==user.m_USER_Number) {
